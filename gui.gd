...
func _usage() -> Label:
	...
	r.text = tr("VTUBE_STUDIO_USAGE_LABEL_TEXT")
...
func _ip_address(toggle: Button) -> HBoxContainer:
	...
	ControlUtil.h_expand_fill(r)
	r.hint_tooltip = tr("VTUBE_STUDIO_IP_ADDRESS_HINT")

	var label := Label.new()
	ControlUtil.h_expand_fill(label)
	label.text = tr("VTUBE_STUDIO_IP_ADDRESS_LABEL_TEXT")
	label.hint_tooltip = tr("VTUBE_STUDIO_IP_ADDRESS_HINT")
	...
	line_edit.hint_tooltip = tr("VTUBE_STUDIO_IP_ADDRESS_HINT")
...
func _toggle_tracking() -> Button:
	...
	r.text = tr("VTUBE_STUDIO_TOGGLE_TRACKING_START")
...
func _on_toggle_tracking(button: Button) -> void:
	...
	for i in trackers.values():
		if i is TrackingBackendTrait and i.get_name() == tr("VTUBE_STUDIO_TRACKER_NAME"):
			tracker = i
			found = true
			break
		
	if found:
		logger.debug("Stopping VtubeStudio tracker")

		tracker.stop_receiver()
		trackers.erase(tracker.get_name())

		button.text = tr("VTUBE_STUDIO_TOGGLE_TRACKING_START")
	else:
		logger.debug("Starting VtubeStudio tracker")
		...
		var vts_tracker = res.unwrap().new()

		trackers[vts_tracker.get_name()] = vts_tracker

		button.text = tr("VTUBE_STUDIO_TOGGLE_TRACKING_STOP")
	
	AM.ps.publish(Globals.TRACKER_TOGGLED, not found, tr("VTUBE_STUDIO_TRACKER_NAME"))
